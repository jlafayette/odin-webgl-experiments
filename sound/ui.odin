...
ui_layout :: proc(ui: ^Ui, container: Container) {
	...
	ui.slider.pos = {w / 2, last_btn_y} - {size.x / 2, 0} + {0, 20} - {14, 0}
	ui.slider.size = size
	ui.slider.min = 0
	ui.slider.max = 100
	if _ui_first {
	...
	ui.checkbox.pos = ui.slider.pos + {0, 40}
	ui.checkbox.size = {200, 24}
	if _ui_first {
...
checkbox_get_shapes :: proc(c: Checkbox) -> (shapes: [7]CheckboxShape) {
